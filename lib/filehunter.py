...
class Metaparser:
    ...
    def __init__(self, domain_name, page_results, exts, download_dir, webdriver):
        ...
        **Parameters**
        ...
        ``domain_name``
            Domain name to search under for files
        ``page_results``
            Limit on the number of search page results to request and parse
        ``exts``
            List of file type extensions for the searches
        ``download_dir``
            Directory to use for the downloaded files
        ``webdriver``
            Selenium webdriver object to use for the web browsing
        """
        self.exts = exts
        self.container = list()
    ...
    def process_file(self, curr_file):
        ...
        **Parameters**
        ...
        # Not a PDF, so treat the current file as an Office doc
            ...
            curr_file = (
                curr_file.replace(" ", "\ ").replace("(", "\(").replace(")", "\)")
            )
            try:
                ...
                created = doc_xml.xpath("//dcterms:created", namespaces=dcterms_ns)[
                    0
                ].text
                last_saved = doc_xml.xpath("//dcterms:modified", namespaces=dcterms_ns)[
                    0
                ].text
                ...
                click.secho(
                    "[!] Failed to extract metadata from {}!".format(curr_file),
                    fg="red",
                )
    ...
    def grab_meta(self):
        ...
        for extension in self.exts:
            ...
            while count < self.page_results:
                try:
                    ...
                    self.browser.get(
                        "https://www.google.com/search?q=site:{}+filetype:{}&start={}".format(
                    ...
                    if "https://www.google.com/recaptcha/api2/anchor" in contents:
                        click.secho(
                            "\n[!] Google returned their reCAPtCHA prompt! File searches cannot be performed.",
                            fg="red",
                        )
                        exit()
        ...
        if len(files) == 0:
            click.secho(
                "[+] No files were located within Google based on the extension(s) and domain you provided.",
                fg="green",
            )
            exit()
        ...
        for item in files:
            ...
            try:
                ...
                click.secho(
                    "[!] There was an error downloading a file from this URL: {}".format(
                        item
                    ),
                    fg="red",
                )
