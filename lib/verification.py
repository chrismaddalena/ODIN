...
def prepare_scope(scope_file, expanded_scope):
    ...
    **Parameters**
    ...
    ``scope_file``
        File containing domain name and IP addresses/ranges
    ``expanded_scope``
        List object for storing to expanded scope list
    """
    try:
        with open(scope_file, "r") as scope_file:
            ...
            click.secho(
                "[+] Scope list expanded to {} items. Proceeding with verification checks.".format(
                    len(expanded_scope)
                ),
                fg="green",
            )
...
def reverse_lookup(target):
    """
    Resolve the provided IP address to a hostname.

    **Parameters**

    ``target``
        Target IP address to resolve
    """
    try:
...
def get_certificate(target):
    """
    Collect SSL/TLS certificate information for the given host.

    **Parameters**

    ``target``
        Domain name to be used for certificate collection
    """
...
def perform_whois(expanded_scope, output):
    ...
    **Parameters**
    ...
    ``expanded_scope``
        List of domain name and IP addresses (no ranges)
    ``output``
        List object for storing the output
    """
    total_addresses = len(expanded_scope)
    with click.progressbar(
        expanded_scope,
        label="Collecting information on addresses",
        length=total_addresses,
    ) as bar:
        for address in bar:
            if not helpers.is_ip(address):
                ...
                try:
                    r = requests.get(
                        "http://whois.arin.net/rest/ip/" + address + ".json", timeout=10
                    )
                    tmp = r.json()
                    try:
...
def print_output(results, report_path):
    ...
    **Parameters**
    ...
    ``report_path``
        File path for the report output
    """
