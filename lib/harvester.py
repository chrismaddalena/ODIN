...
from . import helpers, searchparser


class SearchTwitter:
    """Searching Google using a keyword to find Twitter profiles."""
    ...
    def __init__(self, word, limit):
        ...
        **Parameters**
        ...
        ``word``
            Keyword to use for the Google searches
        ``limit``
            Limit on the number of search results to parse
        """
        self.counter = 0
        self.results = ""
        self.quantity = "100"
    ...
    def do_search(self):
        """Execute a Google search for ``site:twitter.com intitle:on+Twitter`` and a keyword."""
        headers = {"User-Agent": self.user_agent}
        try:
            req = requests.get(
                self.search_url.format(self.counter, self.word),
                headers=headers,
            ...
            click.secho(
                "\n[!] The connection to Google encountered too many redirects!",
                fg="red",
            )
...
class SearchYahoo:
    """Searching Yahoo using a domain name as the keyword to collect email addresses."""
    ...
    def __init__(self, word, limit):
        ...
        **Parameters**
        ...
        ``word``
            Keyword to use for the Yahoo searches
        ``limit``
            Limit on the number of search results to parse
        """
        self.word = word
    ...
    def do_search(self):
        ...
        try:
            req = requests.get(
                self.search_url.format(self.word, self.counter),
                headers=headers,
                timeout=self.requests_timeout,
            )
            self.results = req.content
            ...
            click.secho(
                "\n[!] The connection to Yahoo encountered too many redirects!",
                fg="red",
            )
        except requests.exceptions.RequestException as error:
...
class SearchGoogle:
    """Searching Google using a domain as the keyword to collect email addresses."""
    ...
    def __init__(self, word, limit, start):
        ...
        **Parameters**
        ...
        ``limit``
            Limit on the number of search results to parse
        ``start``
            Search result number to start with for parsing (e.g. 0)
        """
        self.word = word
        self.results = ""
        self.limit = limit
    ...
    def do_search(self):
        ...
        try:
            req = requests.get(
                self.search_url.format(self.quantity, self.counter, self.word),
                headers=headers,
                timeout=self.requests_timeout,
            )
            self.results = req.content
...
class SearchBing:
    """Search Bing using a domain as the keyword to collect email addresses."""
    ...
    def __init__(self, word, limit, start):
        ...
        **Parameters**
        ...
        ``word``
            Keyword to use for the Bing searches
        ``limit``
            Limit on the number of search results to parse
        ``start``
            Search result number to start with for parsing (e.g. 0)
...
class SearchEmailHunter:
    """Search Email Hunter for email addresses tied to a domain."""
    ...
    def __init__(self, domain):
        ...
        **Parameters**
        ...
        ``domain``
            Domain to search for in Email Hunter's database
        """
        self.domain = domain
        try:
            self.emailhunter_api_key = helpers.config_section_map("EmailHunter")[
                "api_key"
            ]
        except Exception:
        ...
        if self.emailhunter_api_key:
            try:
                request = requests.get(
                    self.emailhunter_api_url.format(
                        self.domain, self.emailhunter_api_key
                    ),
                    timeout=self.requests_timeout,
                )
                ...
                if "errors" in results:
                    click.secho(
                        "[!] The request to EmailHunter returned an error!", fg="red"
                    )
                    click.secho("L.. Details: {}".format(results["errors"]), fg="red")
                    return None
                click.secho(
                    "\n[+] Hunter has contact data for {} people.".format(
                        len(results["data"]["emails"])
                    ),
                    fg="green",
                )
            except requests.exceptions.Timeout:
                ...
                click.secho(
                    "\n[!] The connection to Email Hunter encountered too many redirects!",
                    fg="red",
                )
            except requests.exceptions.RequestException as error:
                click.secho(
                    "\n[!] The connection to Email Hunter encountered an error!",
                    fg="red",
                )
                click.secho("L.. Details: {}".format(error), fg="red")
        return results
...
class SearchLinkedIn:
    """Scrape LinkedIn profiles from Bing search results."""
    ...
    # Set the user-agent and URL for the web requests
    search_url = (
        'http://www.bing.com/search?q=site:linkedin.com/in%20"{}"&count=50&first={}'
    )
    user_agent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36"
    ...
    def __init__(self, company, limit=100):
        ...
        **Parameters**
        ...
        ``company``
            Company name to search for on LinkedIn
        ``limit``
            Limit on the number of search results to parse (Default: 100)
        """
        self.limit = limit
    ...
    def do_search(self):
        ...
        while counter < self.limit:
            try:
                headers = {"User-Agent": self.user_agent}
                req = requests.get(
                    self.search_url.format(self.company, counter),
                    headers=headers,
                    timeout=self.requests_timeout,
                )
                ...
                for hit in results:
                    ...
                    if (
                        "/dir/" in link
                        or "/title/" in link
                        or "groupItem" in link
                        or not "linkedin.com" in link
                    ):
                        continue
                    else:
                        profiles[name] = {
...
class Harvester:
    """Execute harvest modules and process the results."""
    ...
    def harvest_all(self, domain):
        """
        Discover email addresses and employee names using search engines like Google, Yahoo, and Bing.

        **Parameters**

        ``domain``
            Domain to search for across all search engines for email addresses and social media profiles
        """
        # Google search
    ...
    def process_harvested_lists(self, harvester_emails, harvester_twitter, hunter_json):
        ...
        **Parameters**
        ...
        ``harvester_emails``
            Email output returned by `harvest_all()`
        ``harvester_twitter``
            Twitter output returned by `harvest_all()`
        ``hunter_json``
            JSON output returned by `SearchEmailHunter.do_search()`
        """
        temp_emails = []
        twitter_handles = []
        harvester_people = []
            ...
            for result in hunter_json["data"]["emails"]:
                ...
                if "first_name" in result and "last_name" in result:
                    if (
                        ...
                        and result["last_name"] is not None
                    ):
        ...
        click.secho(
            "[+] Final unique findings: {} emails, {} people, {} Twitter handles.".format(
                len(unique_emails), len(unique_people), len(unique_twitter)
            ),
            fg="green",
        )
        return (
    ...
    def harvest_twitter(self, handle):
        """
        Review the provided Twitter handle using Tweepy and the Twitter API.

        **Parameters**

        ``handle``
            Twitter handle to check using the Twitter API
        """
        if self.twit_api is None:
            click.secho(
                "[*] Twitter API access is not setup, so skipping Twitter handle lookups.",
                fg="yellow",
            )
        else:
            try:
                ...
                click.secho(
                    "\n[!] Error involving {} -- could be an invalid account.".format(
                        handle
                    ),
                    fg="red",
                )
